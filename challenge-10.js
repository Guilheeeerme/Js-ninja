...
*/
    const isOperatorValid = (operador) => {
        ...
        // É aquele tipo de retorno que por si só já retorna True, ou false se for um elemento falsy
    };
...
*/
    const calculator = (operador) => {
        if (isOperatorValid(operador)){ // ou seja, se for true, a lógica ta no isOperatorValid, false retorna false
            return function(a,b) {
                if (typeof a === 'number' && typeof b === 'number') {
                    return operador[operador](a,b);
                }
            }
        }
        return function(a, b){
    ...
    // Aqui nesse exemplo, as comparações usei para retornar true automaticamente, só passando o elemento truthy
    // Caso sejá falsy, retorna false, não precisei usar if ou switch
...
*/
    const showOperationMessage = (operador,a,b) => {
...
*/
// ?
    const showErrorMessage = (operador) => {
...
*/
    let operationSignal = '^';
    let invalidOperator = calculator(operationSignal);

    if (invalidOperator( number1, number2)) {
