...
- "Eu peso [PESO]Kg."
*/
pessoa.mostrarPeso = () => { return `Olá eu peso ${pessoa.peso}Kg.`}
...
*/
pessoa.nomeCompleto(); // Olá meu nome é Guilherme Moura!
...
inline ao lado da instrução para mostrar qual foi a resposta retornada)
*/
pessoa.mostrarPeso(); // Olá eu peso 60Kg.
...
*/
pessoa.apresentacao = () => {
    // Iniciando a variavel já com comparação
    let sexo = pessoa.sexo === 'Feminino' ? 'a' : 'o';
    let idade = pessoa.idade > 1 ? 'anos' : 'ano';
    let metross = pessoa.caminhouQuantosMetros > 1 ? 'metros' : 'metro';

    return(
    `Olá eu sou ${sexo} ${pessoa.nome} ${pessoa.sobrenome}, tenho ${pessoa.idade} ${idade}, ${pessoa.altura} ,
     meu peso é ${pessoa.peso} e, só hoje, eu já caminhei ${pessoa.caminhouQuantosMetros} ${metross}`);
}
